...
class BaseProcessor:
    ...
    def process_data_for_mongo(self, data: list[Resource]) -> tuple[str, list[dict], list[dict]]:
    ...
    def upload_to_mongo(self, resource_type: str, to_save_data: list[dict], id_references: list[dict]):
    ...
    def process(self, data: list[Resource]):
        ...
        resource_type, to_save_data, id_references = self.process_data_for_mongo(
            data
        )
